...
public class QueryService {
  ...
  public String createQuery(
      ...
      String humanReadable, String explorerUrl, String queryContext, String queryDetails) {
    ...
    tempQuery.setDetails(queryDetails);
    ...
    addOutputToQuery(tempQuery, Optional.ofNullable(outputFormat), Optional.ofNullable(templateId),
        projectType);
  ...
  private String addProjectCodeToUrl(@NotNull String url, @NotNull String projectCode) {
    return (url.contains(ProjectManagerConst.PROJECT_CODE)) ? url :
        ...
        .queryParam(ProjectManagerConst.PROJECT_CODE, projectCode).toUriString();
  }

  private String generateQueryCode() {
    return UUID.randomUUID().toString().replace("-", "")
        .substring(0, ProjectManagerConst.QUERY_CODE_SIZE);
  }
  ...
  public void editQuery(@NotNull Project project,
    ...
    if (projectQuery != null) {
      ...
      if (queryDetails != null) {
        projectQuery.setDetails(queryDetails);
        changedKeyValueMap.put("query details", queryDetails);
      }
      ...
      if (explorerUrl != null) {
        projectQuery.setExplorerUrl(
            addProjectCodeToUrl(decodeUrlIfNecessary(explorerUrl), project.getCode()));
      ...
      if (projectType != null && (outputFormat != null || templateId != null)) {
        ...
        outputFormatOptional.ifPresent(
            of -> changedKeyValueMap.put("output format for project type " + projectType,
                of.toString()));
        templateIdOptional.ifPresent(
            tid -> changedKeyValueMap.put("template id for project type " + projectType, tid));
      }
